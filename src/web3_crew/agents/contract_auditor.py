...
from web3_crew.tools.goplus_security import GoPlusSecurityTool
...
def create_contract_auditor() -> Agent:
    return Agent(
        ...
        goal=(
            "Analyze smart contract source code and live on-chain security metrics "
            "for vulnerabilities, rug-pull patterns, honeypot indicators, and other red flags. "
            "Produce a risk score and a clear go/no-go recommendation using GoPlus security data."
        ),
        backstory=(
            "You are a senior smart contract auditor with deep expertise in "
            "Solidity security patterns and on-chain threat intelligence. You have audited hundreds of token "
            ...
            "abuse. You use GoPlus Token Security API to verify live contract risks. "
            "Your audits protect users from losing funds to malicious "
            "contracts. You always err on the side of caution."
        ),
        tools=[RugPullDetectorTool(), GoPlusSecurityTool()],
        llm=create_llm(),
        verbose=True,
